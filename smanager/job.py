...
class SlurmJob:
    ...
    def __init__(
        ...
        # Use parent.script as experiment name if not provided (for organization only)
        ...
        self.cpus_per_task = cpus_per_task if cpus_per_task is not None else defaults.get('cpus_per_task')
        ...
        self.extra_sbatch_args = extra_sbatch_args or defaults.get('extra_sbatch_args', [])
        self.python_executable = python_executable or defaults.get('python_executable', 'python')
    ...
    def save_script(self, script_dir: Optional[Path] = None) -> Path:
        ...
        Args:
            script_dir: Directory to save the script (uses config default if not provided).
    ...
    def submit(self, dry_run: bool = False) -> Optional[str]:
        ...
        try:
            ...
            raise RuntimeError("sbatch command not found. Is Slurm installed?")
